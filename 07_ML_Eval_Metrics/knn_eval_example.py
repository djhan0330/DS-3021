...
bank_data = pd.read_csv(
    "https://raw.githubusercontent.com/UVADS/DS-3001/main/07_ML_Eval_Metrics/bank.csv")
...
numeric = bank_data.select_dtypes(
...
X_train, X_test, y_train, y_test = train_test_split(
    X, y, train_size=0.70, stratify=y, random_state=21)
...
X_tune, X_test, y_tune, y_test = train_test_split(
    X_test, y_test,  train_size=0.50, stratify=y_test, random_state=49)
...
# This is the probability that our model predicted the correct output based on given inputs, not bad...
print(bank_3NN.score(X_tune, y_tune))
...
# This function gives percent probability for both class (0,1)
bank_prob1 = bank_3NN.predict_proba(X_tune)
# both are important depending on our question, in this example we want the positive class
print(bank_prob1[:5])

# %%
# Now let's retrieve the predictions, based on the tuning set...
bank_pred1 = bank_3NN.predict(X_tune)
# looks good, notice how the probabilities above correlate with the predictions below
print(bank_pred1[:5])

# %%
# Building a dataframe for simplicity, including everything we extracted and the target
final_model = pd.DataFrame(
    {'neg_prob': bank_prob1[:, 0], 'pred': bank_pred1, 'target': y_tune, 'pos_prob': bank_prob1[:, 1]})
...
print(metrics.ConfusionMatrixDisplay.from_predictions(final_model.target,
      final_model.pred, display_labels=[False, True], colorbar=False))
...
print(metrics.RocCurveDisplay.from_predictions(
    final_model.target, final_model.pos_prob))
...
print(metrics.roc_auc_score(final_model.target,
      final_model.pos_prob))  # Looks good!
...
print(metrics.classification_report(
    final_model.target, final_model.pred))  # Nice Work!
